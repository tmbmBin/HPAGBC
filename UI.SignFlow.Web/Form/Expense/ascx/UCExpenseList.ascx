<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UCExpenseList.ascx.cs" Inherits="UI.SignFlow.Web.Form.Expense.ascx.UCExpenseList" %>
...
<ItemTemplate><asp:Literal ID="Num" Text='<%# Eval("請購編號")%>' runat="server" /></ItemTemplate>
</asp:TemplateField>
...
<asp:Button ID="cfm" Text="確認核銷" runat="server" CommandName="Confirm" OnClientClick="if(!confirm('再次確認是否核銷?'))return false;"
 Enabled='<%# Eval("EnableConfirm") %>' />
<asp:Button ID="exp" Text="預支" runat="server" CommandName="Expense" OnClientClick="if(!confirm('確定執行預支?'))return false;" 
