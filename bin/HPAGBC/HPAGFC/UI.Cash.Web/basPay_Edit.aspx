...
'====================================================================================================
	...
	Function get_basPayName(Optional ByVal payCode As String = Nothing) As System.Data.DataSet
		...
		Dim queryString As String = "SELECT " & _
										...
										"N.[payModifyUser], " & _
										"[payModifyDTime], " & _
										"B.[paydDepositorNo], B.[paydDepositor], B.[paydBankSubNo], B.[paydBankName] " & _
									"FROM [vw_basPayName] N " & _
										"LEFT OUTER JOIN [vw_basPayBank] B ON N.payCode = B.payCode "
		If payCode <> Nothing Then
					queryString &= "WHERE (N.[payCode] = @payCode) "
		End If
					queryString &= "ORDER BY N.[payCode]"
'Response.Write(queryString.Replace("@payCode", payCode))
'Response.End
	...
	Function get_basPayBank(Optional ByVal payCode As String = Nothing) As System.Data.DataSet
		...
		Dim queryString As String = "SELECT " & _
										...
										"[paydDepositorNo], " & _
										"[paydDepOfID], " & _
										"ISNULL(NULLIF(RTRIM(paydDefFlag),''),'0') AS [paydDefFlag], " & _
										"ISNULL(NULLIF(RTRIM(paydStopFlag),''),'0') AS [paydStopFlag], " & _
										"[paydStopDate], " & _
										"[paydCreateUsrNo], " & _
										"[paydCreateUser], " & _
										"[paydCreateDTime], " & _
	...
	Function upd_basPayName( _
		...
		Dim queryString As String = "UPDATE [basPayName] SET " & _
									...
									"WHERE ([payCode] = @payCodeOLD)"

		If payCode <> payCodeOLD Then
			queryString &= "; UPDATE [basPayBank] SET [payCode]=@payCode WHERE ([payCode]=@payCodeOLD)"
		End If
		...
		Dim dbCommand As System.Data.IDbCommand = New System.Data.SqlClient.SqlCommand
			...
			dbCommand.Parameters.Add(creatParameter(System.Data.DbType.String, "@payCode", payCode))
			dbCommand.Parameters.Add(creatParameter(System.Data.DbType.String, "@payCodeOLD", payCodeOLD))
	...
	Function del_basPayNameBank(ByVal payCode As String) As Integer
		Dim dbConnection As System.Data.IDbConnection = New System.Data.SqlClient.SqlConnection(connectionString)

		Dim queryString As String = "DELETE FROM [basPayBank] WHERE ([payCode] = @payCode);" & _
									"DELETE FROM [basPayName] WHERE ([payCode] = @payCode)"
		Dim dbCommand As System.Data.IDbCommand = New System.Data.SqlClient.SqlCommand
			dbCommand.CommandText = queryString
			dbCommand.Connection = dbConnection
		...
		If Request.QueryString("mod") <> Nothing Then
			Select Case LCase(Trim(Request.QueryString("mod")))
				Case "edit"
					...
					'dg_basPayName.AllowPaging = False
					dg_basPayName.PageSize = 999
				Case Else
					Editable = False

					'dg_basPayName.AllowPaging = True
					dg_basPayName.PageSize = 10
			End Select
		End If
...
'Dim ds1 As DataSet = get_basPayName()
'Dim dtDistinct As DataTable = ds1.Tables(0).DefaultView.ToTable(True, "payCode","payKind","payID","payName","payAliasNo","payEMail","payZipCode","payAddress","payTalk1","payTel1","payTalk2","payTel2","payShareKind","payEditKind","payDelKind","payCreateUserNo","payCreateUser","payCreateDTime","payModifyUser","payModifyDTime")
'
'			dg_basPayName.DataSource = dtDistinct	'get_basPayName()
'			dg_basPayName.DataBind()
'A21101 改為先不抓
			'A20131 取得傳入的受款人名稱
			If Request.QueryString("cVal") <> Nothing Then
				text_findName.Text = Server.UrlDecode(Trim(Request.QueryString("cVal")))
			End If
	...
	Sub dg_basPayName_ItemDataBound(ByVal sender As Object, ByVal e As DataGridItemEventArgs)
		If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
			If Editable = True Then
				...
				For i As Integer = 2 To e.Item.Cells.Count - 1
'Response.Write(i & " - " & e.Item.Cells(i).Controls(1).ToString & "<br>")
					Select Case i
						...
						Case Else
							...
							Select Case tmp_Object(UBound(tmp_Object))
								...
								Case "Button"
									Dim btn_delete As Button = e.Item.Cells(i).Controls(1)
									btn_delete.Attributes.Add("onclick","return confirm('確定要刪除本筆資料嗎?');")
							End Select
					End Select
				Next i
			End If
		...
		If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
			tmpPayCode = DataBinder.Eval(e.Item.DataItem, "payCode")
...
'Try
'Response.Write(now() & ": " & IsEditing)
'			tmpPayCode = DataBinder.Eval(e.Item.DataItem, "payCode")
'			Dim lbtn_new As LinkButton = e.Item.FindControl("lbtn_new")
...
'Response.Write("[" & sender.Controls(0).Controls(footerIndex-1).FindControl("lbtn_new").ToString)
'Try
'Response.Write(IsEditing)
			Dim lbtn_new As LinkButton = sender.Controls(0).Controls(footerIndex-1).FindControl("lbtn_new")
			If IsEditing = True Then
				lbtn_new.Text = "取消"
			Else
				lbtn_new.Text = "新增"
...
Catch
	'因已過濾過
	'Response.Write(ViewState("RowFilter"))
End Try
...
'				Dim _Label As Label = tc.Controls(2)
'					_Label.Text = "<A style=" & Chr(34) & "FONT-SIZE: x-small; COLOR: #ff8000; MARGIN-TOP: 3; MARGIN-LEFT: 4" & Chr(34) & " href=" & Chr(34) & "javascript:__doPostBack('lbtn_close','')" & Chr(34) & ">尚未建立資料∕關閉</A>"
				...
				Dim lbtn_new As LinkButton = sender.Controls(0).Controls(footerIndex).FindControl("lbtn_new")
					lbtn_new.Text = "取消"
			Catch
			End Try

		Dim div_basPayBank As System.Web.UI.HtmlControls.HtmlGenericControl = Me.FindControl("div_basPayBank")
			div_basPayBank.Attributes("style") = "display: none"

		ElseIf sender.Items.Count > 0 Then
			Dim vCount As Integer = 0
			...
			'990804修正按Cancel後"儲存"顯示為"選取"
			For i As Integer = 0 To sender.Items.Count - 1
				Dim lbtn As LinkButton = sender.Items(i).Cells(0).Controls(0)
				lbtn.Text = IIf(Editable, "儲存", "選取")
			Next i
...
Dim dtDistinct As DataTable = ds1.Tables(0).DefaultView.ToTable(True, "payCode","payKind","payID","payName","payAliasNo","payEMail","payZipCode","payAddress","payTalk1","payTel1","payTalk2","payTel2","payShareKind","payEditKind","payDelKind","payCreateUserNo","payCreateUser","payCreateDTime","payModifyUser","payModifyDTime")
'A00621
'Dim dtDistinct As DataTable = ds1.Tables(0).DefaultView.ToTable(True, "payCode","payKind","payID","payName","payAliasNo","payEMail","payZipCode","payAddress","payTalk1","payTel1","payTalk2","payTel2","payShareKind","payEditKind","payDelKind","payCreateUserNo","payCreateUser","payCreateDTime","payModifyUser","payModifyDTime", "paydDepositorNo", "paydDepositor", "paydBankSubNo", "paydBankName")
dg_basPayName.DataSource = dtDistinct
		Else
			ViewState("payCode").DefaultView.RowFilter = ViewState("RowFilter")
...
'				lbtn = dg_basPayName.Items(i).Cells(0).Controls(0)
'				lbtn.Enabled = True
'			Next i
		'使其可換頁
'		Try
'			Dim footerIndex As Integer = dg_basPayName.Controls(0).Controls.Count-1
'			Dim TC As System.Web.UI.WebControls.TableCell = dg_basPayName.Controls(0).Controls(footerIndex).Controls(0)
...
'====================================================================================================
	...
	Sub dg_basPayName_Select(Sender As Object, E As EventArgs)
		Dim lbtn As LinkButton = Sender.Items(Sender.SelectedIndex).Cells(0).Controls(0)
		Dim text_payCode As TextBox = Sender.Items(Sender.SelectedIndex).FindControl("text_payCode")
		Dim DropDownList1 As DropDownList = Sender.Items(Sender.SelectedIndex).FindControl("DropDownList1")
		If lbtn.Text = "儲存" Then
			Try
				If Trim(text_payCode.Text) = "" Then Response.Write("<script" & ">self.alert('受款人 代碼 為必填。')</" & "script>"): Exit Sub
				If DropDownList1.SelectedIndex <= 0 Then Response.Write("<script" & ">self.alert('受款人 分類 為必選。')</" & "script>"): Exit Sub
...
'Response.Write(" > 0")
		...
		'Response.Write(dg_basPayName.Items(i).Cells(0).Controls(0).ToString() & "<br>")
				...
				'A00628只有一帳戶(或有預設帳戶)則直接傳回
				If dg_basPayBank.Items.Count = 1 Then
					Try
						dg_basPayBank.SelectedIndex = 0
						dg_basPayBank_Select(dg_basPayBank, Nothing)
					Catch
					End Try
				End If
			Else

'				Response.Write("<script" & ">self.alert('尚未建立該人員銀行帳戶資料。')</" & "script>")
'				Exit Sub

				Dim si As Integer = Sender.SelectedIndex
				'受款人代碼
				Dim payCode As String = CStr(Sender.DataKeys(si))
				'受款人統編/身分證號*
				Dim payID As String = CType(Sender.Items(si).FindControl("text_payID"), TextBox).Text
				'受款人名稱
				Dim payName As String = CType(Sender.Items(si).FindControl("text_payName"), TextBox).Text
				'聯絡電話１
				Dim payTel1 As String = CType(Sender.Items(si).FindControl("text_payTel1"), TextBox).Text
				'郵遞區號
				Dim payZipCode As String = CType(Sender.Items(si).FindControl("text_payZipCode"), TextBox).Text
				'地址
				Dim payAddress As String = CType(Sender.Items(si).FindControl("text_payAddress"), TextBox).Text
				'電子郵件
				Dim payEMail As String = CType(Sender.Items(si).FindControl("text_payEMail"), TextBox).Text
				'分類 A21014
				Dim payKind As String = CType(Sender.Items(si).FindControl("DropDownList1"), DropDownList).SelectedValue

				Dim infoStream As String
					infoStream = payCode & "||" & payID & "||" & payName & "||" & payTel1 & "||" & payZipCode & "||" & _
								payAddress & "||||||||||||" & payEMail & "||" & payKind

				If Request.QueryString("popup") <> "" AndAlso Request.QueryString("popup") = "1" Then
					Dim thisID As String = Request.QueryString("thisID")
					Dim replaceID As String = Request.QueryString("replaceID")
					WebScriptFun.Write("js: self.alert('尚未建立該人員銀行帳戶資料。'); window.opener.set_accName('" & thisID & "', '" & replaceID & "', '" & infoStream & "'); window.close();")
				Else
					WebScriptFun.Write("js: self.alert('尚未建立該人員銀行帳戶資料。'); returnValue = '" & infoStream & "'; window.close();")
				End If
	...
	Function BindDetailGrid() As Integer
...
'Response.Write(dg_basPayBank.Items.Count())
		Return dg_basPayBank.Items.Count()
	End Function
	...
	Sub dg_basPayBank_Select(Sender As Object, E As EventArgs)
		...
		'受款人統編/身分證號*
		Dim payID As String = get_tccVAL(dg_basPayName.Items(si).FindControl("hid_payID"))	'CType(dg_basPayName.Items(si).FindControl("text_payID"), TextBox).Text
		'受款人名稱
		...
		'電子郵件
		Dim payEMail As String = CType(dg_basPayName.Items(si).FindControl("text_payEMail"), TextBox).Text
		'分類 A21014
		Dim payKind As String = CType(dg_basPayName.Items(si).FindControl("DropDownList1"), DropDownList).SelectedValue
		...
		Dim paydDepositor As String = CType(dg_basPayBank.Items(si).FindControl("text_paydDepositor"), TextBox).Text
			'帳戶所屬人身份證號*
		Dim paydDepOfID As String = get_tccVAL(dg_basPayBank.Items(si).FindControl("hid_paydDepOfID"))	'CType(dg_basPayBank.Items(si).FindControl("text_paydDepOfID"), TextBox).Text
		...
		If Editable Then
			...
			Try
				...
				Try
					upd_dg_basPayBank()
				Catch
					Response.Write("<script" & ">self.alert('銀行代號、分行代號、帳號 為必填。')</" & "script>")
					Exit Sub
				End Try
		...
		Else
			Dim infoStream As String
				infoStream = payCode & "||" & IIf(paydDepOfID <> "", paydDepOfID, payID) & "||" & payName & "||" & payTel1 & "||" & payZipCode & "||" & _
							payAddress & "||" & paydBankNo & "||" &paydBankSubNo & "||" & paydDepositorNo & "||" & paydBankName & "||" & paydDepositor & "||" & payEMail & "||" & payKind
'Response.Write("<script" & ">alert('" & infoStream & "');</" & "script>")
			...
			If Request.QueryString("popup") <> "" AndAlso Request.QueryString("popup") = "1" Then
				Dim thisID As String = Request.QueryString("thisID")
				Dim replaceID As String = Request.QueryString("replaceID")
				WebScriptFun.Write("js: window.opener.set_accName('" & thisID & "', '" & replaceID & "', '" & infoStream & "'); window.close();")
			Else
...
'====================================================================================================
	...
	Sub btn_find_Click(sender As Object, e As EventArgs)
		...
		Dim RowFilterStr As String = Nothing

		text_findCode.Text = Trim(text_findCode.Text)
		text_findName.Text = Trim(text_findName.Text)
		text_paydDepositorNo.Text = Replace(Trim(text_paydDepositorNo.Text), "-", "")
		text_payTel.Text = Replace(Trim(text_payTel.Text), "-", "")
		text_payAddress.Text = Trim(text_payAddress.Text)
		...
		If Len(text_findCode.Text) > 0 Then
			RowFilterStr = "([payCode] LIKE '" & text_findCode.Text & "') "
		End If
		'身分證
		If Len(text_findID.Text) > 0 Then
			'ds1.Tables(0).DefaultView.RowFilter = "([payID] = '" & text_findID.Text & "') "
			RowFilterStr = "([payID] = '" & text_findID.Text & "') "
		End If
		'姓名/別名/帳戶
		If Len(text_findName.Text) > 0 Then
			If ds1.Tables(0).DefaultView.RowFilter <> "" Then RowFilterStr &= " AND "

			RowFilterStr &= "( ([payName] LIKE '%" & text_findName.Text & "%') " & _
								"OR ([payAliasNo] LIKE '%" & text_findName.Text & "%') " & _
								"OR ([paydDepositor] LIKE '%" & text_findName.Text & "%') ) "
		End If
		'帳號
		If Len(text_paydDepositorNo.Text) > 0 Then
			If ds1.Tables(0).DefaultView.RowFilter <> "" Then RowFilterStr &= " AND "

			RowFilterStr &= "([paydDepositorNo] LIKE '%" & text_paydDepositorNo.Text & "%') "
		End If
		'電話
		If Len(text_payTel.Text) > 0 Then
			If ds1.Tables(0).DefaultView.RowFilter <> "" Then RowFilterStr &= " AND "

			RowFilterStr &= "( ([payTel1] LIKE '%" & text_payTel.Text & "%') " & _
								"OR ([payTel2] LIKE '%" & text_payTel.Text & "%') ) "
		End If
		'地址
		If Len(text_payAddress.Text) > 0 Then
			If ds1.Tables(0).DefaultView.RowFilter <> "" Then RowFilterStr &= " AND "

			RowFilterStr &= "([payAddress] LIKE '%" & text_payAddress.Text & "%') "
		End If

Try
		ds1.Tables(0).DefaultView.RowFilter = RowFilterStr
Catch
	'Response.Write("<HR>" & RowFilterStr)
	Response.Write("執行尋找功能時發生錯誤！")
	Exit Sub
End Try

Dim dtDistinct As DataTable = ds1.Tables(0).DefaultView.ToTable(True, "payCode","payKind","payID","payName","payAliasNo","payEMail","payZipCode","payAddress","payTalk1","payTel1","payTalk2","payTel2","payShareKind","payEditKind","payDelKind","payCreateUserNo","payCreateUser","payCreateDTime","payModifyUser","payModifyDTime")
'Response.Write("<hr>" & dtDistinct.Rows.Count)
		If ds1.Tables(0).DefaultView.Count > 0 Then
			ViewState("payCode") = dtDistinct	'ds1.Tables(0)
			ViewState("RowFilter") = RowFilterStr	'ds1.Tables(0).DefaultView.RowFilter
			dg_basPayName.DataSource = ViewState("payCode")
		Else
			ds1.Tables(0).DefaultView.RowFilter = ""
			dg_basPayName.DataSource = dtDistinct	'ds1
		End If
...
'====================================================================================================
	...
	Function upd_dg_basPayName() As String
		...
		For i As Integer = 0 To dg_basPayName.Items.Count - 1
			...
			Dim payCodeOLD As String = dg_basPayName.DataKeys(dg_basPayName.SelectedIndex)

			If CheckBox2.Checked = True Then
				'刪除
				If del_basPayName(payCodeOLD) <= 0 Then
					'沒刪到
				End If
			...
			Else
				If CheckBox1.Checked = True Then
					'儲存
					Dim payCode As String = get_tccVAL(dgi.FindControl("text_payCode"))
					...
					If CheckBox2.Enabled = True Then
						'修改
						If upd_basPayName(payCodeOLD, payCode, payKind, payID, payName, payAliasNo, _
											payTalk1, payTel1, payTalk2, payTel2, _
											payEMail, payZipCode, payAddress, _
											payShareKind, payEditKind, payDelKind, _
					...
					Else	'新增
						...
						If payCode = -1 Then
							payCode = Val(MsDbFun.GetDbVal("SELECT MAX(CAST(payCode AS int)) FROM basPayName")) + 1
						End If

						If add_basPayName(payCode, payKind, payID, payName, payAliasNo, _
											payTalk1, payTel1, payTalk2, payTel2, _
											payEMail, payZipCode, payAddress, _
											payShareKind, payEditKind, payDelKind, _
											Nothing, _
...
'Response.Write(now() & ": " & IsEditing)
	...
	Sub btn_delete_Click(sender As Object, e As EventArgs)
		Dim payCode As String = sender.CssClass

		Try
			del_basPayNameBank(payCode)
		Catch ex As Exception
			Response.Write("<script" & ">self.alert('" & ex.Message.Replace("\", "\\").Replace(vbCrLf, "\n").Replace("""", "\""").Replace("'", "\'") & "')</" & "script>")
		Finally
			AfterUpdateCancel()
				...
				<table border="0" cellpadding="0" style="border-collapse: collapse">
					<tr>
						...
						<table style="TABLE-LAYOUT: fixed; border-collapse: collapse" cellpadding="0" cellspacing="0" border="1" bordercolor="#D4D0C8">
							<tr>
								<td height="20px" width="90" nowrap align="center" style="display: block; padding-top: 3px"><font size="2">員工編號</font></td>
								<td height="20px" width="50" style="display: block"><asp:TextBox id="text_findCode" runat="server" MaxLength="12" Width="65px" style="TEXT-ALIGN: left; HEIGHT: 21px; margin-top: -1" onfocus="this.select();"></asp:TextBox></td>
								<td height="20px" width="120" nowrap align="center" style="padding-top: 3px"><font size="2">身分證字號(統編)</font></td>
								<td height="20px" width="80"><asp:TextBox id="text_findID" runat="server" MaxLength="12" Width="80px" style="TEXT-ALIGN: left; HEIGHT: 21px; margin-top: -1; IME-MODE: disabled" onfocus="this.select();"></asp:TextBox></td>
								<td height="20px" width="50" nowrap align="center" style="padding-top: 3px"><font size="2">姓名</font></td>
								<td height="20px" width="80"><asp:TextBox id="text_findName" runat="server" MaxLength="10" Width="80px" style="TEXT-ALIGN: left; HEIGHT: 21px; margin-top: -1" onfocus="this.select();"></asp:TextBox></td>
								<td height="20px" width="50" style="display: none" nowrap align="center" style="padding-top: 3px"><font size="2">帳號</font></td>
								<td height="20px" width="80" style="display: none"><asp:TextBox id="text_paydDepositorNo" runat="server" MaxLength="20" Width="80px" style="TEXT-ALIGN: left; HEIGHT: 21px; margin-top: -1" onfocus="this.select();"></asp:TextBox></td>
								<td height="20px" width="50" style="display: none" nowrap align="center" style="padding-top: 3px"><font size="2">電話</font></td>
								<td height="20px" width="80" style="display: none"><asp:TextBox id="text_payTel" runat="server" MaxLength="20" Width="80px" style="TEXT-ALIGN: left; HEIGHT: 21px; margin-top: -1" onfocus="this.select();"></asp:TextBox></td>
								<td height="20px" width="50" style="display: none" nowrap align="center" style="padding-top: 3px"><font size="2">地址</font></td>
								<td height="20px" width="80" style="display: none"><asp:TextBox id="text_payAddress" runat="server" MaxLength="20" Width="80px" style="TEXT-ALIGN: left; HEIGHT: 21px; margin-top: -1" onfocus="this.select();"></asp:TextBox></td>
							</tr>
						</table>
						</td>
				...
				<asp:DataGrid id="dg_basPayName" runat="server" CellPadding="0" BorderStyle="Solid" style="TABLE-LAYOUT: fixed" AutoGenerateColumns="False" BackColor="#C1D1A1" ForeColor="Black" DataKeyField="payCode" OnItemCreated="dg_basPayName_ItemCreated" OnItemDataBound="dg_basPayName_ItemDataBound" OnSelectedIndexChanged="dg_basPayName_Select" AllowPaging="True" PageSize="3" OnPageIndexChanged="dg_basPayName_PageIndexChanged" OnPreRender="dg_basPayName_PreRender" ShowFooter="True">
					...
					<Columns>
						...
						<asp:TemplateColumn HeaderText="代碼">
							<HeaderStyle wrap="False" width="65px" font-bold="True"></HeaderStyle>
							<ItemTemplate>
								<asp:TextBox id="text_payCode" runat="server" style="TEXT-ALIGN: center; margin-top: -1" MaxLength="12" Width="65px" Text='<%# DataBinder.Eval(Container, "DataItem.payCode") %>' tooltip='<%# DataBinder.Eval(Container, "DataItem.payCode") %>' class="removeBorder" onfocus="this.className='applyBorder';this.select();" onblur="this.className='removeBorder'" ReadOnly='<%# Not(Editable) %>'></asp:TextBox>
							</ItemTemplate>
						</asp:TemplateColumn>
						<asp:TemplateColumn HeaderText="分類">
							<HeaderStyle wrap="False" width="65px"></HeaderStyle>
							<ItemTemplate>
								<asp:DropDownList id="DropDownList1" runat="server" SelectedIndex='<%# Sender.Items.IndexOf(Sender.Items.FindByValue(DataBinder.Eval(Container, "DataItem.payKind") & "")) %>' Enabled='<%# Editable %>'>
						...
						<asp:TemplateColumn HeaderText="身分證字號">
							...
							<ItemTemplate>
								<asp:TextBox id="text_payID" runat="server" style="TEXT-ALIGN: left; margin-top: -1" MaxLength="12" Width="85px" Text='<%# maskCheckedIDNo(DataBinder.Eval(Container, "DataItem.payID") ) %>' class="removeBorder" onfocus="this.className='applyBorder';this.select();" onblur="this.className='removeBorder'" ReadOnly='<%# Not(Editable) %>'></asp:TextBox>
								<input type="hidden" id="hid_payID" name="hid_paydDepOfID" runat="server" value='<%# DataBinder.Eval(Container, "DataItem.payID") & "" %>' />
							</ItemTemplate>
						</asp:TemplateColumn>
						<asp:TemplateColumn HeaderText="受款人名稱">
							<HeaderStyle wrap="False" width="200px"></HeaderStyle>
							<ItemTemplate>
								<asp:TextBox id="text_payName" runat="server" style="TEXT-ALIGN: left; margin-top: -1" MaxLength="120" Width="200px" Text='<%# DataBinder.Eval(Container, "DataItem.payName") %>' class="removeBorder" onfocus="this.className='applyBorder';this.select();" onblur="this.className='removeBorder'" ReadOnly='<%# Not(Editable) %>'></asp:TextBox>
							</ItemTemplate>
						</asp:TemplateColumn>
						...
						<asp:TemplateColumn HeaderText="電子郵件信箱">
							...
							<ItemTemplate>
								<asp:TextBox id="text_payEMail" runat="server" style="TEXT-ALIGN: left; margin-top: -1" MaxLength="50" Width="170px" Text='<%# DataBinder.Eval(Container, "DataItem.payEMail") %>' class="removeBorder" onfocus="this.className='applyBorder';this.select();" onblur="this.className='removeBorder'" ReadOnly='<%# Not(Editable) %>'></asp:TextBox>
							</ItemTemplate>
						</asp:TemplateColumn>
						...
						<asp:TemplateColumn HeaderText="移除">
							...
							<ItemStyle wrap="False" horizontalalign="Center"></ItemStyle>
							<ItemTemplate>
								<asp:Button id="btn_delete" runat="server" Text="刪除" Height="21px" Width="35px" Enabled='<%# Editable %>' CssClass='<%# DataBinder.Eval(Container, "DataItem.payCode") %>' onclick="btn_delete_Click"></asp:Button>
								<span style="display: none"><asp:CheckBox id="CheckBox2" runat="server" ToolTip="移除勾選的資料列" Enabled='<%# Editable %>'></asp:CheckBox></span>
							</ItemTemplate>
						</asp:TemplateColumn>
					</Columns>
			...
			<asp:DataGrid id="dg_basPayBank" runat="server" CellPadding="0" BorderStyle="Solid" style="TABLE-LAYOUT: fixed" AutoGenerateColumns="False" BackColor="#C1D1A1" ForeColor="Black" DataKeyField="payCode" OnItemCreated="dg_basPayBank_ItemCreated" OnItemDataBound="dg_basPayBank_ItemDataBound" OnSelectedIndexChanged="dg_basPayBank_Select" OnItemCommand="dg_basPayBank_ItemCommand" OnPreRender="dg_basPayBank_PreRender" ShowFooter="True">
				...
				<Columns>
					...
					<asp:TemplateColumn HeaderText="身分證字號">
						<HeaderStyle wrap="False" width="85px"></HeaderStyle>
						<ItemTemplate>
							<asp:TextBox id="text_paydDepOfID" runat="server" style="TEXT-ALIGN: left; margin-top: -1" MaxLength="12" Width="85px" Text='<%# maskCheckedIDNo(DataBinder.Eval(Container, "DataItem.paydDepOfID") ) %>' class="removeBorder" onfocus="this.className='applyBorder';this.select();" onblur="this.className='removeBorder'" ReadOnly='<%# Not(Editable) %>'></asp:TextBox>
							<input type="hidden" id="hid_paydDepOfID" name="hid_paydDepOfID" runat="server" value='<%# DataBinder.Eval(Container, "DataItem.paydDepOfID") & "" %>' />
						</ItemTemplate>
					</asp:TemplateColumn>
