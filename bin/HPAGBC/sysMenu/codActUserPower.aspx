...
<script runat="server">
'Web.config:<add key="ComponentArtScriptControls" value="CallBack"/>
    Dim connectionString As String = System.Configuration.ConfigurationSettings.AppSettings("SqlDbConnStr")
    'Dim connectionString As String = "Data Source=localhost\sql2012;Initial Catalog='HPAGBC_1030618';Persist Security Info=True;User ID=sa;Password=sqlsa"
        Dim G_usePowerNo As Integer=1
	Function get_members() As System.Data.DataSet
		...
		Dim queryString As String = "SELECT sysBaseUser.PK_usrNo As [mNo], " & _
									...
									"ORDER BY sysBaseUser.DepCode,sysBaseUser.usrName, sysBaseUser.PK_usrNo"
	...
	Function get_groups() As System.Data.DataSet
		...
		Dim queryString As String = "SELECT [dptCode] AS [gNo], " & _
											"[dptName] AS [gName] " & _
									"FROM [codDepartName] " & _
									"ORDER BY 1"
	...
	Function get_memberGroups(ByVal pK_usrNo As Integer) As System.Data.DataTable
		...
		Dim queryString As String = "SELECT useSetBgtDepCode as [PK_grpNo] FROM [codActUserPower] " & _
									"WHERE ([useUsrNo] = @PK_usrNo) " & _
									"AND ([usePowerNo] = @usePowerNo) " & _
									"ORDER BY 1"
		...
		Dim dbParam_usePowerNo As System.Data.IDataParameter = New System.Data.SqlClient.SqlParameter
		dbParam_usePowerNo.ParameterName = "@usePowerNo"
		dbParam_usePowerNo.Value = G_usePowerNo
		dbParam_usePowerNo.DbType = System.Data.DbType.Int32
		dbCommand.Parameters.Add(dbParam_usePowerNo )
	...
	Sub Page_Load(sender As Object, e As EventArgs)
		If Not Page.IsPostBack Then
                        G_usePowerNo =ToolsFun.StrFun.atoi( DB.sysMenu.DAT.sysMenuBZ.Request(“usePowerNo”))  '網址列: &usePowerNo=1
                        IF G_usePowerNo<=0 THEN
                              G_usePowerNo=1
                        End IF 
...
' OnSelectedIndexChanged="cbl_groups_SelectedIndexChanged" AutoPostBack="false"
	...
	Sub btn_update_Click(sender As Object, e As EventArgs)
		Dim mNo As Integer = list_members.SelectedItem.Value
		Dim delStr As String = "DELETE FROM codActUserPower WHERE (useUsrNo= " & mNo & ")"
                delStr  = delStr & " AND ([usePowerNo] = " & G_usePowerNo & "); "
...
'Response.Write(i & ": " & cbl_groups.Items(i).Value & " " & cbl_groups.Items(i).Text & "<br>")
                    ...
                    updStr &= "INSERT INTO codActUserPower " & _
                                "(usePowerNo,useUsrNo, useSetBgtDepCode) " & _
                                "VALUES " & _
                                "(" & _
                                                                G_usePowerNo & ", " & _
                                mNo & ",' " & _
                                cbl_groups.Items(i).Value.Trim & "' " & _
                                ");"
				End If
			Next i
'Response.Write(Replace(delStr & updStr,";",";<br>"))
			Try
				...
				ToolsFun.WebScriptFun.Show("更新 經費單位權限 完畢。")
			Catch Ex As System.Data.SqlClient.SqlException
				ToolsFun.WebScriptFun.Show("更新 經費單位權限 時發生錯誤！" & Ex.Message)
			End Try
		End If
	End Sub
	...
	Public Sub CallBack1_Callback(ByVal sender As Object, ByVal e As ComponentArt.Web.UI.CallBackEventArgs)
		...
		'Literal1.Text = "傳入 " & mNo & "，找到經費單位權限 " & DT.Rows.Count()

		'cbl_groups.RenderControl(e.Output)	'已被DIV包含，故只需div1.RenderControl
                'Literal2.Text="(" & DT.Rows.Count() & ")"
                'Literal2.RenderControl(e.Output)
...
<body style="background-image: url('images/bg_FFFFFF_D4DEF4.gif'); background-repeat: repeat-x; background-position-y: top; background-attachment: fixed" topmargin="0" leftmargin="0" rightmargin="0" bottommargin="0">
	<form runat="server">
				...
				<table border="0" cellpadding="0" cellspacing="0" width="100%" height="45">
					<tr>
						...
						<table border="0" cellpadding="0" cellspacing="0" height="45">
							...
							<tr>
								<td height="21" valign="top" nowrap><p style="margin-top: 0px"><font size="2">人員可管理單位(<%=G_usePowerNo %>)</font></td>
							</tr>
						</table>
						</td>
					</tr>
		...
		<table style="margin-left: 15; margin-top: 15" cellspacing="0" cellpadding="0" border="0">
				...
				<td width="150">
					<table border="0" cellpadding="0" cellspacing="0">
						<tr>
							...
							<td style="padding-left: 1px" nowrap><p style="margin-top: 2px"><font size="2" color="#000080">現有單位</font></td>
						</tr>
					</table>
				</td>
			...
			<tr>
				<td width="150">
					<asp:ListBox id="list_members" runat="server" Height="294px" Width="170px" DataTextField="mName" DataValueField="mNo"></asp:ListBox>
				</td>
				<td align="middle" width="35">
				</td>
				...
				<ComponentArt:Callback id="Callback1" runat="server" OnCallback="CallBack1_Callback">
					<Content>
						<div id="div1" runat="server" style="height: 294px; width: 180px; Overflow: auto; border-style: inset; border-width: 2px; background-color: #FFFFFF">
			...
			<tr>
				...
				<td width="150" style="padding-top: 2px" valign="top">
					<asp:Button id="btn_update" runat="server" Height="22px" Width="150px" Text="儲存" onclick="btn_update_Click"></asp:Button><asp:Literal id="Literal2" runat="server"></asp:Literal>
				</td>
