<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="FrmActPay.aspx.vb" Inherits="UI.ActProdPay.Web.FrmActPay" %>
  	...
  	function ShowProdData()
		{
		    ...
		    {
		        try {   document.all("TRProdData").style.display="block";}
                catch (e){;}
		        try {   document.all("TRProdData1").style.display="block";}
                   catch (e){;}
		    }
		    else {
		       try {  document.all("TRProdData").style.display="none";}
                   catch (e){;}
		       try {  document.all("TRProdData1").style.display="none";}
                   catch (e){;}
		    }
            }
            catch (e) {;}
		...
		function ShowTmpPayData(parSObj, parTObj)
		{
		    ...
		    {
		       try {  document.all("TRTmpPayData").style.display="block";}
                   catch (e){;}
		        try { document.all("TRTmpPayData1").style.display="block";}
                   catch (e){;}
		    }
		    else {
		        try { document.all("TRTmpPayData").style.display="none";}
                   catch (e){;}
		       try {  document.all("TRTmpPayData1").style.display="none";}
                   catch (e){;}
		    }
            }
            catch (e) {;}
	    ...
	    <table border="0" cellpadding="0"cellspacing="0"  style="background-color: Aqua">
	        <tr>
	            <td><asp:Label ID="LBDataMsg" runat="server" Text="" ForeColor="Red" Font-Size="X-Small" ></asp:Label></td>
											   ...
											   <table cellpadding="0" cellspacing="0" border="0" style="table-layout:fixed; width:100%">
                                                    <tr>
                                                        <td style="text-align:left">
                                                            <table cellpadding="0" cellspacing="0" border="0">
                                                                <tr>
                                                                    ...
                                                                    <td style="padding-right: 5px; display:none" nowrap><asp:CheckBox id="CBEPayment" runat="server" Font-Size="X-Small" TextAlign="Right" Text="電子支付" Visible="True" onclick="chkTmpPayAndCash();" /></td>
                                                        ...
                                                        <td style="text-align:right">
                                                            <table cellpadding="0" cellspacing="0" border="0">
                                                                <tr>
                                                                    <td style="padding-right: 5px; display:none" nowrap><asp:CheckBox ID="CBAutoTempPay" runat="server" Font-Size="X-Small" TextAlign="Right" Text="逕行暫付" Visible="false" onclick="ShowTmpPayData(this);chkTmpPayAndCash();" Enabled="False" /></td>
							...
							<!-- ↓區塊(4) --><!-- 區塊(4)↑ --><span id="TRProdData"><asp:PlaceHolder ID="PlaceHolder8" runat="server"></asp:PlaceHolder></span>
							<!-- ↓區塊(5) --><!-- 區塊(5)↑ --><span id="TRProdData1"><asp:PlaceHolder ID="PlaceHolder9" runat="server"></asp:PlaceHolder></span>
							...
							<table cellpadding="0" cellspacing="0" border="0">
								<tr>
									...
									<td><input type="button" id="BtnPrint1" runat="server" 
                                            style="width: 50px; height: 22px" value="列印" visible="False" /></td>
								...
								<tr>
									...
									<table cellpadding="0" style="border-collapse: collapse" border="0">
										<tr>
											...
											<td style="display:none"><asp:dropdownlist id="DDLSortPassFlag" runat="server" AutoPostBack="True">
