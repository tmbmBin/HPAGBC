...
'End If
	...
	'選不到時，先新增再選擇
	Sub ForceSelect(sDDL As Object, sVAL As String, Optional sText As String = "", Optional showStar As Boolean = True)
		Try
			sDDL.SelectedValue = sVAL
			If sDDL.SelectedValue <> sVAL Then Throw New Exception	'故意在V2中引發錯誤
		Catch
			sDDL.Items.Insert(sDDL.Items.Count, New ListItem(IIf(sText <> "", sText, sVAL) & IIf(showStar = True, "*", ""), sVAL))
			sDDL.SelectedValue = sVAL
		End Try
	End Sub

	'設定 ListBox, DropDownList, CheckBoxList, RadioButtonList 單一選取
	Sub ListSelect(sLST As Object, sVAL As String)
		Dim li As ListItem
		For Each li In sLST.Items
			Try
				If li.Value = sVAL & ""
					li.Selected = True
				Else
					Throw New Exception	'故意引發錯誤
				End If
			Catch
				li.Selected = False
			End Try
		Next
	End Sub
	'設定 ListBox, DropDownList, CheckBoxList, RadioButtonList 多重選取
	Sub ListSelect(sLST As Object, sVALs As String())
		Dim li As ListItem
		For Each li In sLST.Items
			For i As Integer = 0 To UBound(sVALs)
				Try
					If li.Value = sVALs(i) & ""
						li.Selected = True
					Else
...
'		Try
			...
			Else
				If Zero2Null = False Then
					Select Case pType
						Case System.Data.DbType.Int32, System.Data.DbType.Currency, System.Data.DbType.Single
...
'		Catch
'			dbParam.Value = System.DBNull.Value
'Response.Write(Now() & " - pName:" & pName)
'Response.Write(" pType:" & pType)
'Response.Write(" pValue:" & pValue)
'		End Try
		Return dbParam
	End Function
...
'Response.Write(i & " - " & Mid(ab, i, 1) & " : " & IsNumeric(Mid(ab, i, 1)) & "<br>")
	...
	Function get_tccVAL(ByRef tcc As Control, Optional ByVal ToolTip As Boolean = False) As Object
		Select Case tcc.GetType.Name	'tcc.ToString()
			...
			Case "DropDownList"	'"System.Web.UI.WebControls.DropDownList"
				Return IIf(ToolTip, CType(tcc, DropDownList).SelectedItem.Text, CType(tcc, DropDownList).SelectedValue)
			...
			Case "HtmlInputCheckBox"	'System.Web.UI.HtmlControls.HtmlInputCheckBox
				Return CType(tcc, HtmlInputCheckBox).Checked
			Case "HtmlInputText"	'System.Web.UI.HtmlControls.HtmlInputText
				Return CType(tcc, HtmlInputText).Value
			Case "HtmlTextArea"	'System.Web.UI.HtmlControls.HtmlTextArea
				Return CType(tcc, HtmlTextArea).Value
			'Case Else
			'	CheckBoxList
			'	RadioButtonList
		End Select
	...
	Function get_tccOBJ(ByRef tcc As Control) As Object
		Select Case tcc.GetType.Name	'tcc.ToString()
			Case "CheckBox"	'"System.Web.UI.WebControls.CheckBox"
				Return CType(tcc, CheckBox)
			Case "Label"	'"System.Web.UI.WebControls.Label"
				Return CType(tcc, Label)
			Case "TextBox"	'"System.Web.UI.WebControls.TextBox"
				Return CType(tcc, TextBox)
			Case "DropDownList"	'"System.Web.UI.WebControls.DropDownList"
				Return CType(tcc, DropDownList)
			Case "HtmlInputHidden"	'HtmlControls.HtmlInputHidden
				Return CType(tcc, HtmlInputHidden)
			Case "HtmlInputCheckBox"	'System.Web.UI.HtmlControls.HtmlInputCheckBox
				Return CType(tcc, HtmlInputCheckBox)
			Case "HtmlInputText"	'System.Web.UI.HtmlControls.HtmlInputText
				Return CType(tcc, HtmlInputText)
			Case "HtmlTextArea"	'System.Web.UI.HtmlControls.HtmlTextArea
				Return CType(tcc, HtmlTextArea)
		End Select
	End Function

'====================================================================================================

	'隱藏身分証尾碼，A123456789 >> A12345****
	Function maskIDNo(ByVal idnoStr As String) As String
		If Session("UserLevel") > 1 Then
			If Len(idnoStr) >= 10 Then
				Return Left(idnoStr, Len(idnoStr) - 4) & Strings.StrDup(4, "*")
			ElseIf Len(idnoStr) >= 3 Then
				Return Left(idnoStr, (Len(idnoStr) - Len(idnoStr) \ 3)) & Strings.StrDup((Len(idnoStr) \ 3), "*")
			Else
				Return idnoStr
			End If
		Else
	...
	'身分證遮罩 A12345678 >> A1*****789
	Public Function ReMarkID(ByVal sID As String) As String
		sID = Trim(sID)
		...
		If Session("UserLevel") > 1 Then
			...
			If Len(sID) >= 7 Then
				...
				tmpID = tmpID & Right(sID, Len(sID) - 7 )
			Else
				tmpID = sID
			End If
	...
	'檢查是否為正確身份證號才隱藏
	Function maskCheckedIDNo(ByVal idnoStr As String, Optional ByVal chkIdNo As Boolean = True) As String
		If Session("UserLevel") > 1 Then
			If chkIdNo = False OrElse CheckSID(idnoStr, True) = True Then
				Return ReMarkID(idnoStr)	'maskIDNo(idnoStr)
			Else
				Return idnoStr	'非身份證號
			End If
		Else
			Return idnoStr
		End If
	...
	'身份證驗證
	Function CheckSID(SID As String, Optional CheckOnly As Boolean = False) As Boolean
		...
		Dim N1 As String = UCase(Left(SID, 1))
		Dim I, N1Ten, N1Unit, Total As Integer
		Dim Pos As Byte = InStr(Letter, N1)

		If Pos <= 0 Then
			If CheckOnly = False Then Response.Write("<script" & ">self.alert('身份證字號第一碼必須為英文字母'); </" & "script>")
			Return False
		End If

		If Len(SID) < 10 Then
			If CheckOnly = False Then Response.Write("<script" & ">self.alert('身分證字號共有10碼'); </" & "script>")
			Return False
		End If

		For I = 2 To 10
			If Not IsNumeric(Strings.Mid(SID, I, 1)) Then
				If CheckOnly = False Then Response.Write("<script" & ">self.alert('身份證第2～9碼必須為數字'); </" & "script>")
				Return False
			End If
		Next I

		If Not (Strings.Mid(SID, 2, 1) = "1" Or Strings.Mid(SID, 2, 1) = "2") Then
			If CheckOnly = False Then Response.Write("<script" & ">self.alert('身份證第2碼必須為1或2'); </" & "script>")
			Return False
		End If

		N1Ten = Strings.Mid(MirrorNumber(Pos - 1), 1, 1)
		N1Unit = Strings.Mid(MirrorNumber(Pos - 1), 2, 1)

		Total = N1Ten * Checker(0) + N1Unit * Checker(1)

		For I = 2 To 10
			Total = Total + Strings.Mid(SID, I, 1) * Checker(I)
		Next I

		If Total Mod 10 = 0 Then

		Else
			If CheckOnly = False Then Response.Write("<script" & ">self.alert('請您正確填寫您的身份證字號'); </" & "script>")
			Return False
		End If
