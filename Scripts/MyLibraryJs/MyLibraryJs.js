...
(function (rootNs) {
    ...
    function fnSetRequired(pArySelectors, pIsRequired) {
        ...
        /// 2013.10.25：Add：針對 MyKendoTimePicker 做特別處理
        /// 2014.04.22：Fix：判斷 MyKendo 相關控制項時，要用完整比對
        ///     如：is("[MyKendoAutoComplete]") 改成 is("[MyKendoAutoComplete='MyKendoAutoComplete']")
        /// </remarks>
        ...
        $jqObject.each(function () {
            ...
            if (pIsRequired) {
                ...
                } else if ($this.is("[MyKendoTimePicker='MyKendoTimePicker']")) {
	...
	var token_DL; //基於資安將動態程式碼固定在一個FUN之內,且不能帶參數
	var CookieName_DL; //基於資安將動態程式碼固定在一個FUN之內,且不能帶參數
    //下載開始，鎖定視窗
	...
	//基於資安將動態程式碼固定在一個FUN之內
	function CloseCookie()
	{
		var cookieValue = $.cookie(CookieName_DL);
			//alert('cookieValue'+ cookieValue);
            if (cookieValue == token_DL)
                fnFinishDownload(CookieName_DL);
    ...
    thisNs.fnKendoWindowOpen = function (src, options) {
        ...
        /// <para>2014.08.01：Edit：由於 IE 效能不佳，關閉特效效果</para>
        /// </remarks>
...
(function (rootNs) {
    ...
    /// 可在 JavaScript 中使用如同 C# 中的 string.format
    /// <para>使用方式 : var fullName = MyLibraryJs.String.Ext.fnFormat('Hello. My name is {0} {1}.', 'FirstName', 'LastName');</para>
    /// mrkt 的程式學習筆記: JavaScript – String.format	-	http://kevintsengtw.blogspot.tw/2011/09/javascript-stringformat.html
    //#region fnFormat
    thisNs.fnFormat = function () {
        /// <summary>
        /// 可在 JavaScript 中使用如同 C# 中的 string.format
        /// <para>使用方式 : var fullName = MyLibraryJs.String.Ext.fnFormat('Hello. My name is {0} {1}.', 'FirstName', 'LastName');</para>
        /// </summary>
        /// <returns type=""></returns>
        rootNs.MathExt.fnFloatAdd()
        var s = arguments[0];
        if (s == null) return "";
    ...
    /// mrkt 的程式學習筆記: JavaScript 擴充String的屬性：startsWith, endsWith, contains	-	http://kevintsengtw.blogspot.tw/2011/09/javascript-stringstartswith-endswith.html
    //#region 字串擴充

    /// 判斷字串開頭是否為指定的字
    thisNs.fnStartsWith = function (source, prefix) {
        /// <summary>
        /// 判斷字串開頭是否為指定的字
        /// </summary>
        /// <param name="source"></param>
        /// <param name="prefix"></param>
        /// <returns type="">回傳: bool</returns>

        return (source.toString().substr(0, prefix.length) === prefix);
    }


    /// 判斷字串結尾是否為指定的字
    thisNs.fnEndsWith = function (source, suffix) {
        /// <summary>
        /// 判斷字串結尾是否為指定的字
        /// </summary>
        /// <param name="source"></param>
        ...
        return (source.toString().substr(this.length - suffix.length) === suffix);
    }


    /// 判斷字串是否包含指定的字
    thisNs.fnContains = function (source, txt) {
        /// <summary>
        /// 判斷字串是否包含指定的字
        /// </summary>
        /// <param name="source"></param>
        ...
        return (source.toString().indexOf(txt) >= 0);
    }
    //#endregion
})(MyLibraryJs);
