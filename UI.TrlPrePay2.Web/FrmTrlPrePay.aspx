<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="FrmTrlPrePay.aspx.vb" Inherits="UI.TrlPrePay2.Web.FrmTrlPrePay" %>
...
//    //strParams = "&acmyear=" + <%=Session("default_Year") %> + "&acmNo=" + document.all("TBAcmWordNum").title + "&acmwordnum=" + document.all("TBAcmWordNum").value + "&acmNo1=0" + "&acpPayYear=" +  <%=Session("default_Year") %>  + "&acmkidnum=<%=strDefKidNum  %>" + "&page1qty=7&page2qty=43" ;
        ...
        //2015 10/23 調整
        $(function () {
            $("#<%= CBAutoShouldPay.ClientID%>").hide(); //應付隱藏
            $('label[for="CBAutoShouldPay"]').hide();//應付隱藏
            $('#TRTmpPayData1').hide(); //附記事項隱藏
