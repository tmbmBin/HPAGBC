<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="PrePay.Web.FrmPrePay_tsbPrePay.Default" %>
    ...
    <form id="form1" runat="server">
        <div>
                ...
                <table style="width: 100%">
                    <tr>
                        ...
                        <td class="qury_table_td3">
                            ...
                            <asp:DropDownList ID="ddlAtsbPlanCode" runat="server" Width="650px" Style="display: none">
                            </asp:DropDownList>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr class="qury_table_tr">
                        <td colspan="2">
                            <span style="color: white">只列出</span>
            ...
            <div style="text-align: center; margin: 10px;">
                ...
                &nbsp;&nbsp;
                <input type="button" id="btnViewExchNo" value="查看傳票" class="Btn02" />
                ...
                <table style="width: 100%; border: 0; background-color: black;">
                    <tr>
                        ...
                        <td style="text-align: center;">
                            <asp:Label ID="Label1" Text="預撥案件登錄資料明細(請依單一頁次進行審核)" runat="server" ForeColor="White" />
                        </td>
                        ...
                        <Columns>
                            ...
                            <asp:TemplateField HeaderText="年度" Visible="True">
                                <ItemTemplate>
                                    ...
                                    <asp:Label ID="lblCtsbPrePayYear" runat="server" Text='<%# Eval("CtsbPrePayYear")??0 %>' Style="display: none"></asp:Label>
                                </ItemTemplate>
                                <HeaderStyle HorizontalAlign="Center" Width="30" />
                                <ItemStyle HorizontalAlign="Center" Width="30" />
                            </asp:TemplateField>
                            ...
                            <asp:TemplateField HeaderText="預撥號" Visible="True">
                                <ItemTemplate>
                                    <asp:Label ID="lblCtsbPrePayNo" runat="server" Text='<%# Eval("CtsbPrePayNo")  %>'></asp:Label>
                                </ItemTemplate>
                                <HeaderStyle HorizontalAlign="Center" Width="45" />
                                <ItemStyle HorizontalAlign="Center" Width="45" />
                            </asp:TemplateField>
                            ...
                            <asp:TemplateField HeaderText="審核狀態" Visible="True">
                                <ItemTemplate>
                                    <asp:Label ID="lblPassFlagStats" runat="server" ToolTip='<%#  Eval("CtsbPassDTime").ToRocDate() %> ' Text='<%# ( (Eval("CtsbPassFlag")??"0").ToString().Equals("1")? "已審核":"未審核" )  %>'></asp:Label>
                                </ItemTemplate>
                                <HeaderStyle HorizontalAlign="Center" Width="55" />
                                <ItemStyle HorizontalAlign="Center" Width="55" />
                            </asp:TemplateField>
                              ...
                              <asp:TemplateField HeaderText="審核日期" Visible="True">
                                <ItemTemplate>
                                  <asp:TextBox ID="txttsbPassDTime" runat="server" MyKendoDatePicker="MyKendoDatePicker"  Width="150px" Text='<% # PassDateFormat( (Eval("CtsbPassDTime")??"").ToString() , (Eval("CtsbPassFlag")??"0").ToString()  )%>'   Enabled='<%# (Eval("CtsbPrePayNo").ToString()=="0")? false:true  %>' ></asp:TextBox>
                                </ItemTemplate>
                                <HeaderStyle HorizontalAlign="Center" Width="55" />
                                <ItemStyle HorizontalAlign="Center" Width="55" />
                            </asp:TemplateField>
                            ...
                            <asp:TemplateField HeaderText="是否審核" Visible="True">
                                <ItemTemplate>
                                    <asp:CheckBox ID="chkBtsbPassFlag" runat="server" Checked='<%#  (Eval("CtsbPassFlag")??"0").ToString().Equals("1")%> ' Width="40px" Enabled='<%# (Eval("CtsbPrePayNo").ToString()=="0")? false:true  %>' />
                                </ItemTemplate>
        ...
        <script type="text/javascript">
            $(function () {
                ...
                $("#btnNew").click(function () {
                    ...
                    if (selectedTr.length > 0) {
                        ...
                        var CtsbPrePayNo = selectedTr.prop("title").split("_")[3];
                        var CtsbPrePayYear = selectedTr.prop("title").split("_")[4];

                        var keys = { BtsbYear: BtsbYear, BtsbApplyNo: BtsbApplyNo, BtsbDecideNo: BtsbDecideNo, CtsbPrePayNo: CtsbPrePayNo, CtsbPrePayYear: CtsbPrePayYear };
                ...
                $("#btnPrint").click(function () {
                    ...
                    if (selectedTr.length > 0) {
                        ...
                        var src = "PrePayPrint2.aspx?menuID=" + menuID + "&formStatus=View"
                            + "&Keys=" + encodeURIComponent(JSON.stringify(keys));
                        MyLibraryJs.JqPluginExt.fnKendoWindowOpen(src,
                            {
                                title: "列印資料",
                ...
                $("#btnsetPass").click(function () {
                    ...
                    $("#<%=GridView1.ClientID%> tr").not(':first').each(function () {
                        ...
                        var tmpCheck = $(this).find("[id$=chkBtsbPassFlag]").is(':checked'); //勾選否
                        var tmpCheckEnable = $(this).find("[id$=chkBtsbPassFlag]").prop("disabled");//是否可以勾選
                        var tmptsbApplyNo = $.trim($(this).find("[id$=lblBtsbApplyNo]").text()); //會簽號                                          
                        var tmpDecideNo = $.trim($(this).find("[id$=lblBtsbDecideNo]").text()); //核定號
                        var tmptsbPrePayNo = $.trim($(this).find("[id$=lblCtsbPrePayNo]").text()); //預撥號
                        var tmptsbPrePayYear = $.trim($(this).find("[id$=lblCtsbPrePayYear]").text()); //預撥年度
                        ...
                        if (tmpCheckEnable.toString() == "false") {
                            ...
                            if (tmpStatus == "已審核" && tmpCheck.toString() == "false") {
                                var tsbPrePay = new Object()
                                tsbPrePay.ApplyNo = tmptsbApplyNo;
                                tsbPrePay.DecideNo = tmpDecideNo;
                                tsbPrePay.PrePayNo = tmptsbPrePayNo;
                                tsbPrePay.tsbPrePayYear = tmptsbPrePayYear; //預撥年度
                                tsbPrePay.tsbPassDTime = tmptsbPassDTime; //核銷日期
                                tsbPrePay.IsCheck = tmpCheck;

                                ArrayData.push(tsbPrePay);
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            alert(result.msg);
                            location.href = location.href + encodeURI('&WIN=0');
                        },
            ...
            //#region $("#btnViewExchNo").click 看傳票
            $("#btnViewExchNo").click(function () {
                var selectedTr = $$("##hlkSelect:contains('V')").parentsUntil("tr").parent();
                if (selectedTr.length > 0) {
                    // var menuID = $("#hdfMenuID").val();
                    //20150918 [mdf]: 修改為 讀取 資料庫 參數
                    //var menuID = '149A'
                    var menuID = $("#hdfRptMenuID").val();
                    ...
                    var CtsbPrePayNo = selectedTr.prop("title").split("_")[3];
                    var CtsbPrePayYear = selectedTr.prop("title").split("_")[4];
                    var AtsbApplyWordNum = selectedTr.find("[id$=lblAtsbApplyWordNum]").text();

                    var keys = { BtsbYear: BtsbYear, BtsbApplyNo: BtsbApplyNo, BtsbDecideNo: BtsbDecideNo, CtsbPrePayNo: CtsbPrePayNo, CtsbPrePayYear: CtsbPrePayYear };


                    var src = "showExchNo.aspx?Keys=" + encodeURIComponent(JSON.stringify(keys));

                    //var src = "../../RptInputData.Web/FrmRptInputData_tsbRptInputDataM/Default.aspx?menuID=" + menuID
                    //    + "&tsbApplyWordNum=" + tsbApplyWordNum;
                    MyLibraryJs.JqPluginExt.fnKendoWindowOpen(src,
                        {
                            title: "傳票瀏覽(會簽編號/預撥號)：" + AtsbApplyWordNum + "/" + CtsbPrePayNo,
                            width: document.getElementsByTagName('body')[0].clientWidth * 0.95,
                            height: document.documentElement.offsetHeight * 0.85
                        });
                } else {
