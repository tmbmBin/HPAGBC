...
$('###ID##_FileData').uploadify({
	'uploader': '##ROOT##../../HPAGFC/FUMS/Kernel//uploadify.swf','script': '##ROOT##../../HPAGFC/FUMS/Kernel//upload.ashx','scriptData':{PK:##ID##_PK},
	'cancelImg': '##ROOT##../../HPAGFC/FUMS/Kernel//cancel.png',
	'buttonText': '',
	'buttonImg': '##ROOT##../../HPAGFC/FUMS/Kernel//button.png',
	'hideButton':false,
	'width': 64,
	'height': 14,
...
function ##ID##_loadUploaded() {
	...
	$.ajax({
		type: "POST", url: "##ROOT##../../HPAGFC/FUMS/Kernel//managex.ashx", dataType: "json", data: p,
		success: function (data) {
			...
			for (var i = 0; i < d.length; i++) {
				...
				//var c= ena=="1"? "<div class='cancel'><a id='" + id + "' s='" + s + "' href=''><img src='##ROOT##../../HPAGFC/FUMS/Kernel//cancel.png' border='0'></a></div>":"";
				var c= ena=="1"? "<div class='cancel'><a id='" + id + "' s='" + s + "' href=''><img src='##ROOT##../../HPAGFC/FUMS/Kernel//"+ (s=="trash"?"trash01.gif":"trash02.png").toString()+ "' border='0' title='"+(s=="trash"?"立即刪除":"移至資源回收桶")+"'></a></div>":"";
				var cancel = $(c+"<span class='fileName'>"
					+(dl=="1"? "<a href='##ROOT##../../HPAGFC/FUMS/Kernel//filex/?v=" + id.substring(0, 12) + "&s=" + s + "&k=" + id.substring(12) + "' class='"+s+ "' target='_blank'>":"")
					+ fileName + ' (' + bs + ")"+(dl=="1"?"</a>":"")
		...
		error: function () {alert("刪除檔案發生錯誤...");}
	});
}
function ##ID##_remove_click(e){
    ...
    $.ajax({
	    type: "POST", async: false, url: "##ROOT##../../HPAGFC/FUMS/Kernel//managex.ashx", data: pp,
	    success: function (data) {
...
function ##ID##_trashClear_click(){
	...
	if(confirm("清空全部檔案?\n\n"+"警告:此動作會將檔案徹底刪除且無法復原，是否確定刪除?")){
		...
		$.ajax({
			type: "POST", async: true, url: "##ROOT##../../HPAGFC/FUMS/Kernel//managex.ashx", data: p,
			success: function (d) {
				##ID##_loadUploaded();
			},
