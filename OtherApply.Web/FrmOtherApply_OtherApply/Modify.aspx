                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        $.each(result.codOutlayCodeItem, function (i, value) {
                            ...
                            if (value.ILevel != "99")
                                ddl2.append("<option value=\"" + IValue + "\" style=\"color:Navy\" disabled=\"true\" label=\"" + IText + "\"></option>")
                            else {
                                if (select_type == 0) // 讓預設值跳到可選的項目上。
                                {
                                    select_type = 1;
                                    ddl2.append("<option value=\"" + IValue + "\" selected=\"true\" label=\"" + IText + "\"></option>")
                                }
                                else ddl2.append("<option value=\"" + IValue + "\" label=\"" + IText + "\"></option>")
                            }
                        });
                    }
                });
